                   ...
                   ifstream &sailingInFile)
{
    ...
    bool exitFlag = false;
    while (true)
    { // This loop controls the whole checkIn process
        string sailingID;
        while (true)
        { // This loop gets the licensePlate from the user
        ...
        string phoneNumber;
        while (true)
        { // This loop gets the phone number from the user
        ...
        { // Vehcile already exists on file, use it's data to fill out the booking
            getVehicleDimensions(licensePlate, &vehicleLength, &vehicleHeight, inFile);
        }
        else
        { // Vehicle does not already exist on file, gather data and create it
            string vehicleLength;

            while (true)
            { // This loop gets the vehicle length from the user
            ...
            string vehicleHeight;
            while (true)
            { // This loop gets the vehicle height from the user
...
)
{
    ...
    { // This loop searches for a booking
             ...
             ifstream &sailingInFile)
{
    ...
    { // This loop controls the whole checkIn process
        ...
        { // This loop gets the licensePlate from the user
            ...
            if (sailingID.empty())
            {
                return; // Return to main-menu if nothing is entered
            }
            // If the sailing ID doesn't match the correct format, let them try again
            else if (sailingID.length() != 9 || sailingID[3] != '-' || sailingID[6] != '-' ||
                     !isalpha(sailingID[0]) || !isalpha(sailingID[1]) || !isalpha(sailingID[2]) ||
        ...
        if (exists)
        { // Gets the booking's details so it can be re-created as a checked-in vehicle
            string line;
            while (getline(inFile, line))
            { // This loop gets the parameters from the file
                stringstream ss(line);
